...
export class SitiosformComponent implements OnInit {
  ...
  validarcamposSitio(){
    this.sitioForm = this.formBuilder.group({
      ...
      'sitsuperficietotal' : ['',[Validators.required,Validators.pattern("^[0-9]*$"),Validators.maxLength(5)]],
      ...
      'sitcontacto' : ['',[Validators.required,Validators.maxLength(10),Validators.pattern("^[0-9]*$")]],
      'sitcontactocel' : ['',[Validators.required,Validators.maxLength(10),Validators.pattern("^[0-9]*$")]]
    });
  }
   ...
   obtenerCantones(id){
     ...
     this.sitioServicio.getCantonesxId(id).subscribe((data)=>{
        this.cantones = data;
     },error =>{console.log(error); this.openDialog('Error, en consumo de servicios sicpas','cancel');});
   }
   obtenerParroquias(id){
    if(id == undefined){return false;}
    this.sitioServicio.getParroquiasId(id).subscribe((data)=>{
      this.parroquias = data;
    },error =>{console.log(error); this.openDialog('Error, en consumo de servicios sicpas','cancel');});
   }

   cargaDivisionPolitica(id){
     this.sitioServicio.getRecursivaDivisionPolitica(id).subscribe((data)=>{
  ...
  openDialog(mensaje,icono) : void{
     ...
     dialogRef.afterClosed().subscribe(result=>{
       this.router.navigate(['registros/sitios']);
     });
   }
   ...
   editarDatos():void{
    ...
    },error =>{console.log(error); this.openDialog('Error, verifique su conexión a Internet','cancel');});
   }

   guardarSitios(formulario){
    this.sitioServicio.guardarSitio(formulario).subscribe((data)=>{        
      if(data.sitid > 0){
        this.openDialog('Datos Guardados correctamente..!','check_circle');
      }else{
        this.openDialog('NO se pudo guardar los datos..!','error_outline');
      }
    },error => {
      console.log("produjo un error"+error); 
      console.log(error); 
      this.openDialog('Error en el servicio de envio los datos..!','cancel');
    });
   }
}
